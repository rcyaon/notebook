...
The goal was simple to say and brutally hard to do: build a battery powered chip small enough to tag a wild bird, and accurate enough to actually record its call. Everything I worked on, the ADC designs, the switching schemes, the custom logic, was really just chasing that one sentence.
...
Kestrel was my first project, and the first time I took something all the way through tapeout, fabrication, and got back real silicon that actually worked. It's an 8-bit SAR ADC, fully differential, built in TSMC's 180nm process.
...
The chip worked, which honestly still feels like a small miracle in analog design, where one unbudgeted parasitic can quietly wreck months of work. Here's what I measured on silicon:
...
The best part, though, was proving the whole concept worked end to end. I digitized and reconstructed a Scrub Jay call through the ADC, and the resulting spectrogram clearly showed the bird's frequency modulated chirps. That's the entire thesis of the project validated in a single plot.

### The design choices that made it work

None of this happens without a handful of deliberate tradeoffs made early on.

**Binary weighted CDAC, no split capacitor.** The capacitor DAC is really the heart of any SAR ADC, it holds and moves the sampled charge during each bit decision. A split capacitor array saves area but adds parasitic capacitance at the split node that hurts linearity, not just gain. Since keeping full 8-bit ENOB was non negotiable, I went with a plain binary weighted array instead. Parasitics still show up, but they only cost gain, which calibrates out, instead of corrupting the actual bits.

**IMCS switching.** This is the algorithm that decides how charge moves on and off the CDAC at each step, and it's one of the biggest levers on both capacitor count and energy. IMCS only needs an (n-1)-bit CDAC instead of the full array, which roughly halves both capacitor count and switching energy compared to a conventional SAR. For something meant to run off a coin cell on a bird, that matters a lot.

**Bottom plate sampling.** Charge injection from the sampling switches is easy to underestimate until it shows up in your linearity plot. Bottom plate sampling makes that injected charge signal independent, and since Kestrel is fully differential, whatever's left cancels out as common mode instead of messing with the actual signal.

**Double tail comparator with kickback cancellation.** The comparator only draws current during an actual decision, not while idling, so it's zero static power. The annoying part with any dynamic comparator is kickback, where the regeneration phase kicks charge back onto the inputs and disturbs the voltage you're trying to measure. I added cancellation transistors sized at half the input pair width to inject an opposing pulse and cancel that out.

**A synchronous FSM.** Less exciting, but probably the most pragmatic call in the whole design. A synchronous state machine drives the bit decisions, which keeps everything robust across process, voltage, and temperature corners. If timing ends up marginal after fabrication, the fix is just clocking it slower. That's a recoverable failure, which matters a lot when you only get one shot at silicon as an undergrad.
...
Simple to draw. Much harder to make hold 7.4 effective bits on real silicon.

---

## after Kestrel: chasing power in two directions

**Fall 2025, Spring 2026 (ongoing)**

Once Kestrel proved the team could actually tape out a working ADC, the obvious next question was how much further this could go. Instead of picking one direction, the next phase split into two parallel projects: one chasing speed, one chasing the lowest power possible. I also ended up leading recruiting and training for the incoming cohort, which turned into its own kind of design problem. How do you compress two years of SAR ADC intuition into one semester of onboarding?
...
**Pulse width sensitivity.** The first version used a level triggered "go" signal, and it got inconsistent right around 5 ns pulse widths, which happened to be exactly the timescale this design needed. I ended up building a custom edge-triggered SR latch from scratch, which got rid of the sensitivity completely instead of just patching around it.

**Precharge coupling.** The comparator's precharge window (how long it resets between decisions) was originally set implicitly by logic propagation delay, around 6 ns. That's a problem because propagation delay shifts with process corner, so the window would quietly shrink or grow depending on the fab run. I added a separate delay line path to control precharge independently, which stretched the window from ~6 ns out to ~23 ns and made it corner independent.
...
There's a pretty clean arc across all three projects. Kestrel taught me the fundamentals, and more importantly, the 10x lesson about what simulation does and doesn't catch. The Flash ADC pushed as far as possible on power, in a space nobody on the team had touched before. The async SAR pushed on speed and timing robustness, and exposed failure modes, pulse sensitivity, corner dependent precharge, that only show up once you take the clock's safety net away.

Different axes, same question underneath it all: how small, how efficient, and how robust can you make a data converter before it's ready to ride along on something that flies.